// src/components/Conv2DLayerNode.jsx
import React, { useState, useEffect } from 'react';
import { NodeProps, Handle, Position } from 'reactflow';

export type Conv2DLayerNodeData = {
  inputSize: [number, number, number]; // [Cin, H, W]
  filters: number; // Cout
  kernelSize: [number, number]; // [kH, kW]
  stride: [number, number]; // [sH, sW]
  padding: [number, number]; // [pH, pW]
  dilation: [number, number]; // [dH, dW]
  bias: boolean;
};
...
const Conv2DLayerNode: React.FC<NodeProps<Conv2DLayerNodeData>> = (props) => {
  ...
  const [outputSize, setOutputSize] = useState<[number, number, number]>([0, 0, 0]);
  const [isCollapsed, setIsCollapsed] = useState(true);

  useEffect(() => {
    const [Cin, Hin, Win] = props.data.inputSize;
    const [pH, pW] = padding;
    const [dH, dW] = dilation;
    const [kH, kW] = kernelSize;
    const [sH, sW] = stride;
    
    const Hout = Math.floor((Hin + 2 * pH - dH * (kH - 1) - 1) / sH + 1);
    const Wout = Math.floor((Win + 2 * pW - dW * (kW - 1) - 1) / sW + 1);

    setOutputSize([filters, Hout, Wout]);
  ...
  return (
    <div className="bg-white shadow-md rounded border border-gray-300 w-72 relative">
      ...
      >
        <img
          src="convolutions/conv2d.png"
          alt="Conv2D"
          className="rounded-full h-10 w-10 mr-4"
        />
        <div className="flex-grow font-semibold text-gray-800 text-left">Conv2D</div>
        ...
        <div className="p-4 border-t border-gray-200">
          ...
          <div className="mb-2 flex items-center">
            ...
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={kernelSize[0]}
              onChange={(e) => setKernelSize([Number(e.target.value), kernelSize[1]])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Height"
            />
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={kernelSize[1]}
              onChange={(e) => setKernelSize([kernelSize[0], Number(e.target.value)])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Width"
            />
            <div className="relative ml-2 group">
          ...
          <div className="mb-2 flex items-center">
            ...
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={stride[0]}
              onChange={(e) => setStride([Number(e.target.value), stride[1]])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Height"
            />
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={stride[1]}
              onChange={(e) => setStride([stride[0], Number(e.target.value)])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Width"
            />
            <div className="relative ml-2 group">
              ...
              <div className="hidden group-hover:block absolute bottom-full mb-2 w-64 p-2 bg-white border border-gray-300 rounded shadow-lg text-xs text-gray-700 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                The step size for moving the kernel. Larger strides reduce the output size.
              </div>
            </div>
          </div>
          <div className="mb-2 flex items-center">
            ...
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={padding[0]}
              onChange={(e) => setPadding([Number(e.target.value), padding[1]])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Height"
            />
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={padding[1]}
              onChange={(e) => setPadding([padding[0], Number(e.target.value)])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Width"
            />
            <div className="relative ml-2 group">
          ...
          <div className="mb-2 flex items-center">
            ...
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={dilation[0]}
              onChange={(e) => setDilation([Number(e.target.value), dilation[1]])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Height"
            />
            <input
              ...
              className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              value={dilation[1]}
              onChange={(e) => setDilation([dilation[0], Number(e.target.value)])}
              style={{ padding: '10px', height: '40px' }}
              placeholder="Width"
            />
            <div className="relative ml-2 group">
          ...
          <div>
            ...
            <div className="mt-1 block w-full rounded border-gray-300 shadow-sm sm:text-sm bg-gray-100 p-2">
              {`(${outputSize[1]}, ${outputSize[2]}, ${filters})`}
            </div>
          </div>
        </div>
      )}
