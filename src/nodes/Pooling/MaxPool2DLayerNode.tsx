...
export type MaxPool2DLayerNodeData = {
...
const MaxPool2DLayerNode: React.FC<NodeProps<MaxPool2DLayerNodeData>> = (props) => {
  ...
  const calculateOutputDimension = (inputDim: string, height : boolean) => {
    const cleanedInputDim = inputDim.replace(")", "").replace(/\s+/g, '');
    if (!isNumeric(cleanedInputDim) || kernelSize === null || stride === null || padding === null || dilation === null) {
      return height ? "Hout" : "Wout";
    }
    return Math.floor(((parseInt(cleanedInputDim) + 2 * padding - dilation * (kernelSize - 1) - 1) / stride) + 1).toString();
  };
  ...
  useEffect(() => {
    ...
    if (inputShape) {
      ...
      inputShapeArr[inputShapeArr.length - 2] = calculateOutputDimension(inputShapeArr[inputShapeArr.length - 2], true);
      inputShapeArr[inputShapeArr.length - 1] = calculateOutputDimension(inputShapeArr[inputShapeArr.length - 1], false);
  ...
  return (
    <div className="bg-white shadow-md rounded border border-gray-300 w-72 relative">
      ...
      <NodeHeader title="MaxPool2D Layer" isCollapsed={isCollapsed} setIsCollapsed={setIsCollapsed} imageSource="pooling/maxpool2d.png" imageAlt="MaxPool2D" props={props} />
      {!isCollapsed && (
        <div className="p-4 border-t border-gray-200">
          ...
          <ShapeLabel input={false} shape={props.data.outputShape || 'Not Connected'} shapeHintMessage="The shape of the output tensor is expected to be (Batch, Channels, Output Height, Output Width)." />
        </div>
      )}
...
export default MaxPool2DLayerNode;
